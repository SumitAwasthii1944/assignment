...
export function SubTopicFormModal({ open, initialName = "", onClose, onSubmit }: SubTopicFormModalProps) {
    ...
    return (
        <Modal open={open} title={initialName ? "Edit sub-topic" : "Add sub-topic"} onClose={onClose}>
            <label className="mb-1 block text-sm font-medium text-slate-700">Sub-topic name</label>
            <input
                autoFocus
                value={name}
            ...
            <div className="mt-5 flex justify-end gap-2">
                ...
                >
                    {initialName ? "Save changes" : "Add sub-topic"}
                </button>
            </div>
        </Modal>
    );
