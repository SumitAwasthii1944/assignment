...
const buildQuestionHierarchy = (
    ...
    const topicOrderMap = new Map(
        sheet.config.topicOrder.map((topic, index) => [topic, index]),
    );
    const topics = new Map<string, NestedTopic>();

    for (const question of questions) {
        const sheetId = question.sheetId as string
        const topicName = question.topic as string;
        const subTopicName = (question.subTopic as string | null) ?? "Uncategorized";
...
export const importCodolioQuestions = asyncHandler(
    async (_req: Request, res: Response) => {
        const codolioResponse: CodolioResponse = await fetchCodolioSheet();
        const questions = codolioResponse.data.questions;
        const { topicOrder, subTopicOrder, questionOrder } =
            codolioResponse.data.sheet.config;
        const questionOrderMap = new Map(
            questionOrder.map((questionId, index) => [questionId, index]),
        );
        ...
        const subTopicNamesByTopic = new Map<string, string[]>();
        ...
        const subTopicOperations = [...subTopicNamesByTopic.entries()].flatMap(
            ([topic, subTopics]) =>
                subTopics.map((name, order) => ({
                    updateOne: {
                        filter: {
                            sheetId: codolioResponse.data.sheet._id,
                            topic,
...
export const reorderQuestion = asyncHandler(async (req: Request, res: Response) => {
    const { id, prevId, nextId } = req.body as {
        id?: string;
        prevId?: string | null;
        nextId?: string | null;
    };
    ...
    await QuestionModel.bulkWrite(
        remainingQuestions.map((item, order) => ({
            updateOne: {
                filter: { _id: item._id },
                update: { $set: { order } },
            },
