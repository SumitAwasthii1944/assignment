...
const getSheetId = (req: Request) => {
    const { sheetId } = req.body as { sheetId?: unknown };
...
export const getSubTopic = asyncHandler(async (req: Request, res: Response) => {
...
export const editSubTopic = asyncHandler(async (req: Request, res: Response) => {
    const subTopic = await SubTopicModel.findById(req.params.id);
    const name = typeof req.body.name === "string" ? req.body.name.trim() : "";
...
export const deleteSubTopic = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (questionCount > 0) {
        return res.status(409).json(
            new ApiResponse(409, null, "Delete the subtopic's questions first"),
        );
    }
...
export const reorderSubTopic = asyncHandler(async (req: Request, res: Response) => {
    ...
    await SubTopicModel.bulkWrite(
        remainingSubTopics.map((item, order) => ({
            updateOne: {
                filter: { _id: item._id },
                update: { $set: { order } },
            },
