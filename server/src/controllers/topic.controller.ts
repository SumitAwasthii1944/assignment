...
export const editTopic = asyncHandler(async (req: Request, res: Response) => {
    const name = typeof req.body.name === "string" ? req.body.name.trim() : "";
    const topic = await TopicModel.findById(req.params.id);
    ...
    if (name !== topic.name) {
        await QuestionModel.updateMany(
            { sheetId: topic.sheetId, topic: topic.name },
            { $set: { topic: name } },
        );
        await SubTopicModel.updateMany(
            { sheetId: topic.sheetId, topic: topic.name },
            { $set: { topic: name } },
        );
    }
...
export const deleteTopic = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (questionCount > 0 || subTopicCount > 0) {
        return res.status(409).json(
            new ApiResponse(409, null, "Delete the topic's questions and subtopics first"),
        );
    }
...
export const reorderTopic = asyncHandler(async (req: Request, res: Response) => {
    ...
    await TopicModel.bulkWrite(
        remainingTopics.map((item, order) => ({
            updateOne: {
                filter: { _id: item._id },
                update: { $set: { order } },
            },
